...
var refreshLabel = function(map, vehicles) {
	return function() {
		...
		for(var i = 0; i < vehicles.length; i++) {
			v = vehicles[i];
			v.marker_.setLabel(new BMap.Label(vehicles[i].obj_name, {
...
bmap.prototype.addStopMarker = function(lon, lat, stop_content, p) {
	...
	stop_marker = new BMap.Marker(latLng, {
		icon: icon
	});
	//打开该车辆的信息窗体
...
bmap.prototype.addLocMarker = function(lon, lat, title, content) {
	...
	var latLng = new BMap.Point(lon, lat);
	loc_marker = new BMap.Marker(latLng, {
		icon: icon
	});
...
bmap.prototype.addStartMarker = function(lon, lat, title, content) {
	...
	start_marker.setLabel(new BMap.Label(title, {
		offset: new BMap.Size(30, 0)
	}));
	start_marker.getLabel().setStyle({
...
bmap.prototype.addEndMarker = function(lon, lat, content) {
	...
	end_marker.setLabel(new BMap.Label(content, {
		offset: new BMap.Size(15, 0)
	}));
	end_marker.getLabel().setStyle({
		border: "1px solid red"
	});
...
bmap.prototype.addCenterMarker = function(lon, lat) {
	...
	var center_marker = new BMap.Marker(latLng, {
		icon: icon
	});
...
bmap.prototype.addVehicles = function(vehicles, is_infowin, is_playback) {
	...
	for(var i = 0; i < vehicles.length; i++) {
		...
		if(v != null) {
			...
			v.marker_ = new BMap.Marker(latLng, {
				icon: icon
			});
			v.marker_.setRotation(vehicles[i].activeGpsData.direct);
			v.marker_.setLabel(new BMap.Label(vehicles[i].vehicleName, {
				offset: new BMap.Size(30, 0)
			}));
			v.marker_.getLabel().setStyle({
				border: "0px solid red",
				backgroundColor: "transparent",
				fontWeight: "bold",
				fontFamily: "PingFang SC",
...
var markerClickFunction = function(v) {
	...
	return function(e) {
		...
		geocoder.getLocation(latLng, geoFn, {
			"poiRadius": "500",
			"numPois": "10"
		});
		select_vehicle = _v;
			...
			case TOOL_TYPE_POI:
				...
				$("#add_hobbySave").on("click", function() {
					...
					_adSaveDate = {
						...
						lon: current_marker.getPosition().lng,
...
bmap.prototype.addPoi = function(poi) {
	...
	if(p != null) {
		...
		p.marker_.setLabel(new BMap.Label(poi.name, {
			offset: new BMap.Size(20, 0)
		}));
		p.marker_.getLabel().setStyle({
			border: "0px solid red",
			color: "#666666",
			backgroundColor: "transparent",
			fontWeight: "bold",
...
bmap.prototype.showGeo = function(poi) {
	...
	current_retangle = new BMap.Polygon(bps, {
		strokeColor: "#FF0000",
		strokeWeight: 2,
		strokeOpacity: 0.8
	});
